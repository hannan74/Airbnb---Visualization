...
st.set_page_config(page_title= "Airbnb Data Visualization",
                   ...
                   menu_items={'About': """# This dashboard app is created by *Hannanstreamlit run  *!
                                        Data has been gathered from mongodb comapss"""}
                  )
...
def datafr():
    df= pd.read_csv("D:/Guvi/Capstone/Airbnb.csv")
    return df
...
with st.sidebar:
    st.title(":green[CAPSTONE PROJECT-4]")
...
if options == "Home":
    ...
    image1= Image.open("D:/Guvi/Capstone/title_airbnb.png")
...
if options == "Data Exploration":
    ...
    with tab1:
        st.title("**PRICE ANALYSIS**", anchor= False)
        ...
        with col1:
            ...
            fig_bar= px.bar(df_bar, x='property_type', y= "price", title= "PRICE FOR PROPERTY_TYPES",hover_data=["number_of_reviews","review_scores"],
                            color_discrete_sequence=px.colors.sequential.Redor_r, width=600, height=600)
        ...
        with col3:
            ...
            fig_do_bar = px.bar(df_do_bar, x='bed_type', y=['minimum_nights', 'maximum_nights'], 
                                ...
                                barmode='group',color_discrete_sequence=px.colors.sequential.Rainbow, width=600, height=600)
        ...
        with col4:
            ...
            fig_do_bar_2 = px.bar(df_do_bar_2, x='bed_type', y=['bedrooms', 'beds', 'accommodates'], 
                                    ...
                                    barmode='group',color_discrete_sequence=px.colors.sequential.Rainbow_r, width= 600, height= 600)
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv("D:/Guvi/Capstone/Airbnb.csv")
            return df_a

        df_a= datafr()

        st.title("**AVAILABILITY ANALYSIS**", anchor= False)
        col1,col2= st.columns(2)
        ...
        with col1:
            ...
            df_a_sunb_30= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_30",width=600,height=600,
        ...
        with col2:
            ...
            df_a_sunb_60= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_60",width=600,height=600,
        ...
        with col3:
            ...
            df_a_sunb_90= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_90",width=600,height=600,
                                        title="Availability_90",color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
            st.plotly_chart(df_a_sunb_90)

        with col4:

            df_a_sunb_365= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact"], values="availability_365",width=600,height=600,
                                        title="Availability_365",color_discrete_sequence=px.colors.sequential.Greens_r)
            st.plotly_chart(df_a_sunb_365)
        
        roomtype_a= st.selectbox("Select the Room_Type", df2_a["room_type"].unique())
    ...
    with tab3:
        ...
        st.title("**LOCATION ANALYSIS**", anchor= False)
        st.write("")
        ...
        country_l= st.selectbox("Select_the Country",df_l["country"].unique())
        ...
        fig_1= px.bar(df_val_sel_gr, x="accommodates", y= ["cleaning_fee","bedrooms","beds"], title="ACCOMMODATES",
                    hover_data= "extra_people", barmode='group', color_discrete_sequence=px.colors.sequential.Rainbow_r,width=1200, height= 550)
        st.plotly_chart(fig_1)
        
        
        room_ty_l= st.selectbox("Select The Room_Type", df_val_sel["room_type"].unique())
    ...
    with tab4:
        ...
        st.title("**GEOSPATIAL VISUALIZATION**", anchor= False)
        st.write("")

        fig_4 = px.scatter_mapbox(df, lat='latitude', lon='longitude', color='price', size='accommodates',
                        color_continuous_scale= "Inferno",hover_name='name',range_color=(0,49000), mapbox_style="open-street-map",
                        zoom=1)
    ...
    with tab5:
        ...
        property_ty_t= st.selectbox("Select the PROPERTY_TYPE",df1_t["property_type"].unique())
        ...
        room_type_t= st.selectbox("Select the ROOM_TYPE",df2_t_sorted["room_type"].unique())
...
if options == "About":
    ...
    st.write('''***Gather data from Airbnb's public API or other available sources. I have Gathered the data from the dataset given by GUVI Team.
